...
export const ChatView: React.FC = () => {
  ...
  return (
    <div className="glass-card rounded-4xl border border-white dark:border-white/10 shadow-elevated flex flex-col h-[700px] overflow-hidden">
      ...
      <div className="bg-obsidian-950 dark:bg-[#070A12] text-white px-4 py-2.5 flex flex-wrap items-center justify-between gap-2 text-xs border-b border-white/10 shrink-0">
        ...
        <div className="flex items-center gap-1.5 text-coral-300 font-bold text-[11px]">
          ...
          <strong className="text-white bg-white/15 dark:bg-white/10 px-2.5 py-0.5 rounded-full border border-white/10">{chatPartnerName} ({chatPartnerRole})</strong>
        </div>
      ...
      <div className="p-4 sm:px-6 bg-white dark:bg-[#0F172A] border-b border-obsidian-200 dark:border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0 backdrop-blur-md">
        <div className="flex items-center gap-3 text-left">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-coral-50 dark:bg-coral-950/70 text-coral-600 dark:text-coral-400 border border-coral-200 dark:border-coral-800/60">
                {chatPartnerRole}
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 self-end sm:self-center">
          {targetDog && (
            <button
              ...
              className="bg-obsidian-100 dark:bg-slate-800 hover:bg-obsidian-200 dark:hover:bg-slate-700 text-obsidian-900 dark:text-white border border-obsidian-200 dark:border-slate-700 px-3.5 py-2 rounded-full text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer shadow-xs shrink-0"
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-sky-50 dark:bg-sky-950/70 hover:bg-sky-500 dark:hover:bg-sky-600 text-sky-700 dark:text-sky-300 hover:text-white border border-sky-200 dark:border-sky-800/60 px-3.5 py-2 rounded-full text-xs font-bold transition-all shadow-xs shrink-0 cursor-pointer"
          >
      ...
      {/* MESSAGES FEED */}
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-[#F2EDE4] dark:bg-[#080C16] text-left">
        ...
        {activeMessages.map(msg => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`max-w-xs sm:max-w-md rounded-3xl p-4 text-left ${
                  isMine
                    ? 'bg-gradient-to-tr from-coral-600 to-coral-500 text-white rounded-br-xs shadow-md'
                    : 'bg-white dark:bg-[#161F33] text-obsidian-900 dark:text-slate-100 border border-obsidian-200/80 dark:border-slate-700/60 rounded-bl-xs shadow-sm'
                }`}
              >
                {!isMine && (
                  <div className="text-[10px] font-black text-coral-600 dark:text-coral-400 mb-1 uppercase tracking-wider">
                    {msg.senderName}
                  </div>
                )}
                ...
                {msg.isDogBark ? (
                  <div className={`flex items-center gap-2 py-1 px-3 rounded-full font-black text-xs ${
                    isMine ? 'bg-white/20 text-white' : 'bg-coral-50 dark:bg-coral-950/70 text-coral-700 dark:text-coral-300 border border-coral-200 dark:border-coral-800/60'
                  }`}>
        ...
        {isTyping && (
          <div className="flex items-center gap-2 justify-start animate-in fade-in duration-200">
            ...
            <div className="bg-white dark:bg-[#161F33] px-4 py-2.5 rounded-3xl text-xs font-semibold text-obsidian-700 dark:text-slate-200 border border-obsidian-200 dark:border-slate-700/60 flex items-center gap-2 shadow-sm">
      ...
      {/* QUICK SUGGESTIONS */}
      <div className="px-4 py-2.5 bg-white dark:bg-[#0F172A] border-t border-obsidian-200 dark:border-white/10 flex items-center gap-2 overflow-x-auto shrink-0">
        ...
        {quickPrompts.map((p, i) => (
          <button
            ...
            className="text-xs font-bold px-3.5 py-1.5 rounded-full bg-obsidian-100 dark:bg-[#1A2234] hover:bg-coral-50 dark:hover:bg-coral-950/60 text-obsidian-700 dark:text-slate-200 hover:text-coral-600 dark:hover:text-coral-300 border border-obsidian-200 dark:border-slate-700/60 shrink-0 transition-colors cursor-pointer"
          >
            {p}
          </button>
        ))}
      ...
      >
        <button
          ...
          className="w-11 h-11 rounded-2xl flex items-center justify-center bg-obsidian-100 dark:bg-[#1A2234] hover:bg-coral-50 dark:hover:bg-coral-950/60 text-obsidian-700 dark:text-slate-200 hover:text-coral-500 dark:hover:text-coral-400 transition-colors cursor-pointer border border-obsidian-200 dark:border-slate-700/60 shrink-0"
        >
        ...
        <button
          ...
          className="w-11 h-11 rounded-2xl flex items-center justify-center bg-coral-50 dark:bg-coral-950/70 hover:bg-coral-100 dark:hover:bg-coral-900/50 text-coral-600 dark:text-coral-400 transition-colors cursor-pointer border border-coral-200 dark:border-coral-800/60 shrink-0"
        >
        ...
        <input
          ...
          className="flex-1 px-4 py-3 rounded-2xl bg-obsidian-100 dark:bg-[#1A2234] border border-obsidian-200 dark:border-slate-700/80 focus:bg-white dark:focus:bg-[#1A2234] focus:border-coral-500 focus:ring-4 focus:ring-coral-100 dark:focus:ring-coral-500/20 text-obsidian-900 dark:text-white placeholder:text-obsidian-400 dark:placeholder:text-slate-400 text-xs sm:text-sm outline-hidden font-semibold"
        />
