...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sendMessage = (convId: string, text: string, image?: string, isDogBark?: boolean) => {
    ...
    // 🤖 Intelligent Automated Multi-Turn Response from the Dog's Real Guardian / Applicant
    const activeConv = conversations.find(c => c.id === convId);
    let targetDog = dogs.find(d => d.id === activeConv?.dogId);

    if (!targetDog) {
      targetDog = dogs.find(d => convId.includes(d.id) || convId.toLowerCase().includes(d.name.toLowerCase()));
    }
    if (!targetDog && dogs.length > 0) {
      targetDog = dogs[0]; // fallback
    }
    ...
    if (targetDog) {
      setTimeout(() => {
        ...
        } else if (lowerText.includes('vaccin') || lowerText.includes('health') || lowerText.includes('medical') || lowerText.includes('booklet') || lowerText.includes('shot')) {
        ...
        } else if (lowerText.includes('park') || lowerText.includes('meet') || lowerText.includes('weekend') || lowerText.includes('time') || lowerText.includes('schedule') || lowerText.includes('kab') || lowerText.includes('kahan')) {
          replyText = `That sounds great! We can easily meet at a nearby pet-friendly park around 5:00 PM. Please click the "Park Meetup" button at the top of our chat to confirm the time and place! 🌳🐾`;
        } else if (lowerText.includes('food') || lowerText.includes('treat') || lowerText.includes('diet') || lowerText.includes('eat') || lowerText.includes('khana')) {
        ...
        } else if (lowerText.includes('routine') || lowerText.includes('friendly') || lowerText.includes('kid') || lowerText.includes('child') || lowerText.includes('cat') || lowerText.includes('bache')) {
        ...
        } else if (lowerText.includes('adopt') || lowerText.includes('process') || lowerText.includes('paper') || lowerText.includes('form') || lowerText.includes('agreement')) {
          replyText = `The adoption process on PawConnect is 100% smooth! Once we meet at the park and verify chemistry with ${targetDog.name}, we can digitally sign the agreement and issue the official certificate. 📜✨`;
        } else {
          const generalReplies = [
            `Thanks for your message! ${targetDog.name} is very energetic today and loves making new friends. Let me know when you'd like to meet! 🐕`,
            `That's great! ${targetDog.name} is fully house-trained and loves outdoor walks. Do you have any other questions about their daily routine? 🐾`,
            `Wonderful! I'm happy to answer any questions you have about ${targetDog.name}. We can also schedule a park meet & greet whenever you are free! 🌳`
          ];
          const randomIndex = Math.abs(text.length + Date.now()) % generalReplies.length;
          replyText = generalReplies[randomIndex];
        }

        const replySenderId = senderId === targetDog.currentOwnerId ? 'user_sarah' : targetDog.currentOwnerId;
        const replySenderName = senderId === targetDog.currentOwnerId ? 'Sarah Jenkins (Applicant)' : (targetDog.currentOwnerName || 'Dog Guardian');
        const replySenderAvatar = senderId === targetDog.currentOwnerId ? 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=400' : (targetDog.currentOwnerAvatar || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=400');
